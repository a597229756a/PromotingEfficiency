...
func run(ctx context.Context, emailClient email.MailService, handler *email.XLSXAttachmentHandler, logger *storage.Logger, cronSpec string, dcfg *config.DataConfig, cfg *config.Config) {
	select {
	case <-ctx.Done():
	...
	default:
		...
		// 查询email是否有更新
		newEmail, err := email.CheckAndProcessEmails(ctx, emailClient, handler, logger)
		...
		if err != nil {
			...
			dfw, err := email.ReadXlsx(bytes, cfg.SheetName)
			if err != nil {
...
func worker(ctx context.Context, handler *email.XLSXAttachmentHandler, newEmail *email.Email, logger *storage.Logger, dfw *email.DataFrameWrapper, dcfg *config.DataConfig, cfg *config.Config) {
